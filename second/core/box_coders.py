...
class BevBoxCoder(BoxCoder):
    ...
    def code_size(self):
        ...
        custom_ndim = self.custom_ndim + res
        return custom_ndim

    def _encode(self, boxes, anchors):
        anchors = anchors[..., [0, 1, 3, 4, 6]]
        boxes = boxes[..., [0, 1, 3, 4, 6]]
        box_encode = box_np_ops.bev_box_encode(boxes, anchors, self.vec_encode, self.linear_dim)
        return box_encode
    ...
    def _decode(self, encodings, anchors):
        ...
        concat = np.concatenate([ret[..., :2], z_fixed, ret[..., 2:4], h_fixed, ret[..., 4:]], axis=-1)
        return concat
